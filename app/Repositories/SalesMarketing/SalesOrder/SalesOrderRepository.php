...
class SalesOrderRepository extends BaseRepository
{
    ...
    {
        $this->table = table('sales', 'sales-order.master');
    }
    ...
    public function all(?array $filters = null): Builder
    {
        ...
        if (!isset($filters['from']) || !isset($filters['to'])) {
            ...
            $salesOrderDetail = table("sales", "sales-order.detail");
            $product = table("master", "product");
            ...
            JoinBuilder::leftJoin(
                ...
                $this->table->pk(),
                '=',
                $salesOrderDetail->column('sales_order_hd_id')
            );

            JoinBuilder::leftJoin(
                $data,
                $product,
                $salesOrderDetail->column('product_id'),
                '=',
                $product->pk()
            );
        }

        return $data->whereBetween($this->table->column('sales_order_date'), [$filters['from'], $filters['to']]);
    }
